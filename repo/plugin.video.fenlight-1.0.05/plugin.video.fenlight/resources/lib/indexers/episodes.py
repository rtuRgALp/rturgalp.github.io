...
nextep_include_unaired = settings.nextep_include_unaired
		...
		try:
			...
			if list_type_starts_with('next_'):
				...
				if highlight_color: display = '%s: [COLOR%s]%sx%s - %s[/COLOR]' % \
					...
					display = '%s%s: %s%sx%s - %s%s' % (display_premiered, upper(title), highlight_start, str_season_zfill2, str_episode_zfill2, ep_name, highlight_end)
				...
				display = '[%s] %s: %sx%s - %s' % (display_premiered, upper(title), str_season_zfill2, str_episode_zfill2, ep_name)
			else: display = '%s: %sx%s - %s' % (upper(title), str_season_zfill2, str_episode_zfill2, ep_name)
