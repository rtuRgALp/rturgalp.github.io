...
class TraktCache:
	...
	def delete(self, string):
		try: self.dbcon.execute(TC_BASE_DELETE, (string,))
		except: pass

class TraktWatched(TraktCache):
	def __init__(self):
		TraktCache.__init__(self)
...
def clear_trakt_hidden_data(list_type):
	...
	try: dbcon.execute(DELETE, (string,))
	except: pass
...
def clear_trakt_list_contents_data(list_type):
	...
	try: dbcon.execute(DELETE_LIKE % string)
	except: pass
...
def clear_trakt_calendar():
	dbcon = trakt_cache.dbcon
	try: dbcon.execute(DELETE_LIKE % 'trakt_get_my_calendar_%')
	except: return
