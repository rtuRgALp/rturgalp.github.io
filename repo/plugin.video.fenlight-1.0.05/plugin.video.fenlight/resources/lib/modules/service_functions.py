		...
		while not monitor.abortRequested():
			try:
				...
				if status in ('success', 'no account'):
					logger('Fen Light', trakt_service_string % ('Success', success_line_dict[status]))
					if get_setting('fenlight.trakt.refresh_widgets', 'false') == 'true': kodi_refresh()
				elif status == 'failed': logger('Fen Light', trakt_service_string % ('Failed. Error from Trakt', next_update_string))
...
class AutoRun:
	def run(self):
		logger('Fen Light', 'AutoRun Service Starting')
		if auto_start():
			try: run_addon()
			except: return logger('Fen Light', 'AutoRun Service Failed')
		return logger('Fen Light', 'AutoRun Service Finished')
